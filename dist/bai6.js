...
while (true) {
    ...
    if (inputs === 'C') {
        ...
        let ip = Number(prompt("Bạn muốn update vị trí nào trong carts ? Hãy nhập vào dưới đây"));
        if (ip < 0 && ip > carts.length) {
            console.log("Không có vị trí này trong carts");
        }
        else {
            let ipsl = Number(prompt("Nhập vào số lượng muốn thay đổi"));
            for (let i in stores) {
        ...
        let ips = Number(prompt("Bạn muốn delete vị trí nào trong carts ? Hãy nhập vào dưới đây"));
